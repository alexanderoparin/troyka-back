...
- **Автоматическая синхронизация данных** - username и firstName обновляются при каждом взаимодействии с ботом или входе через Telegram Login Widget
...
### 🖼️ Стили генерации
- `GET /api/art-styles` - Получение списка доступных стилей из БД (отсортированы по id)
...
### ✨ Улучшение промптов
- `POST /api/prompt/enhance` - Улучшение промпта с помощью ИИ (DeepInfra API, Gemini 2.5 Flash)
...
- `GET /api/admin/payments` - Список всех платежей с данными пользователей из Telegram
...
#### DeepInfra API (улучшение промптов)
- `deepinfra_api_key` - API ключ для DeepInfra
- `deepinfra_max_tokens` - Максимальное количество токенов для ответа (по умолчанию: 1500)
- `deepinfra_temperature` - Температура для генерации (по умолчанию: 0.5)
...
- `style_id` - ссылка на стиль из art_styles (опционально)
- `description` - описание изображения от ИИ (опционально)
- `created_at` - дата создания

### 🖼️ Стили изображений (art_styles)
- `id` - уникальный идентификатор стиля (первичный ключ)
- `name` - название стиля (например "Реалистичный", "Аниме", "Без стиля")
- `prompt` - промпт для добавления к основному промпту пользователя

### 👤 Стили пользователей (user_styles)
- `user_id` - ссылка на пользователя (первичный ключ)
- `style_id` - ссылка на стиль из art_styles
- `created_at` - дата создания
- `updated_at` - дата обновления
...
- `current_prompt` - текущий промпт для генерации
- `input_image_urls` - JSON массив URL входных изображений
- `waiting_style` - флаг ожидания выбора стиля
- `last_generated_message_id` - ID последнего сообщения с генерацией
- `created_at` - дата создания
- `updated_at` - дата обновления
...
- **FileCleanupService** - автоматическая очистка старых неиспользуемых файлов (по расписанию)
- **ImageProxyService** - проксирование изображений от Fal.ai
- **PricingService** - тарифные планы
- **ArtStyleService** - управление стилями генерации (с кешированием на 30 минут)
- **PromptEnhancementService** - улучшение промптов через DeepInfra API (Gemini 2.5 Flash)
- **TelegramBotService** - обработка команд Telegram бота с синхронизацией данных пользователя
- **TelegramAuthService** - авторизация через Telegram с обновлением данных
- **AdminService** - админ-панель (статистика, платежи, пользователи)
...
### Очистка файлов
- Автоматическая очистка старых неиспользуемых файлов по расписанию (ежедневно в 03:00)
- Удаляются файлы старше 30 дней, которые не используются в истории генераций
- Проверяются только файлы из `input_image_urls` (внешние URL из `image_urls` не проверяются)
- Логируются только удаления и ошибки

### Синхронизация данных Telegram
- При каждом взаимодействии с Telegram ботом или входе через Telegram Login Widget данные пользователя синхронизируются
- Обновляются только изменившиеся поля (username, firstName, photoUrl)
- Сравнение полей перед обновлением для оптимизации запросов к БД

## 📋 Последние изменения

### v1.7.0 - Админ-панель и синхронизация Telegram данных
- ✅ **Админ-панель** - API для просмотра статистики, платежей и пользователей
- ✅ Роль-ориентированный доступ через Spring Security (только ADMIN)
- ✅ **Синхронизация данных пользователя из Telegram** - автоматическое обновление username, firstName при каждом взаимодействии
- ✅ Сравнение полей перед обновлением для оптимизации запросов к БД
- ✅ **FileCleanupService** - автоматическая очистка старых неиспользуемых файлов (по расписанию, ежедневно в 03:00)
- ✅ Улучшенная обработка ошибок контент-фильтра DeepInfra
- ✅ Оптимизация кода (убраны дублирования, улучшен стиль)

### v1.6.0 - Улучшение промптов и переход на styleId
- ✅ Улучшение промптов с помощью ИИ через DeepInfra API (Gemini 2.5 Flash)
- ✅ Поддержка анализа изображений при улучшении промпта
- ✅ Учет выбранного стиля при улучшении промпта
- ✅ Переход на использование `style_id` вместо `style_name` в БД
- ✅ Кеширование стилей на 30 минут для оптимизации
- ✅ API для сохранения/получения стиля пользователя
- ✅ Кастомный exception `PromptEnhancementException` для обработки ошибок
- ✅ Глобальная обработка ошибок улучшения промптов
